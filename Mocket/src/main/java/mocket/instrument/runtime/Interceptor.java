...
public class Interceptor {
            ...
            while(!stop) {
                try {
                    ...
                    if(m.sid != Interceptor.sid) {
                        logger.error("Receive host's response!");
                        stop();
                    } else {
    ...
    public synchronized static int collectParams(String... params){
        ...
        Interceptor.params.put(actionId, param);
        return actionId;
    }
    ...
    public static boolean notifyAndBlock(String action, int actionId) {
        ...
        messageBuffer.putInt(ActionType.valueOf(action).getValue()); // Action type

        messageBuffer.putInt(actionId); // Blocking id
        setSemaphore(actionId, true);

        HashMap<String, String> param = params.get(actionId);
        if (!params.get(actionId).isEmpty()) {
