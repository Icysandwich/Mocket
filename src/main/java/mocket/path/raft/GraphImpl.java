    ...
    public ArrayList<Transition> readGraph(String edgePath, String nodePath) {
        ...
        try{
            ...
            while((str = in_edge.readLine()) != null) {
                ...
                Transition root = new Transition(null, -1, new BehaviorImpl(), nodes.get(edges[0]));
                rootNodes.add(root);
...
//                Behavior behavior = Behavior.NULL;
                for (int i = 1; i < edges.length; i++) {
                    if (i % 2 == 0){ // is node ID
                        ...
                        prev = new Transition(prev, sid, new mocket.path.zk.BehaviorImpl(BehaviorType.valueOf(behavior), sid), nodes.get(edges[i]));
                    } else { // is behavior
                        behavior = edges[i];
                    }
                }
